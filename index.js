			...
			timer = setTimeout(async () => {
				try {
					...
					let data = await fetch(
						`https://api.spotify.com/v1/search?q=${query}&type=track,artist&limit=5&access_token=${spotifyAccessToken}`
					).then((res) => res.json());
					resolve(data);
				} catch (error) {
					reject(error);
...
async function getTopTracks() {
	const response = await fetch(
		`https://ws.audioscrobbler.com/2.0/?method=chart.gettoptracks&api_key=${API_key}&format=json&limit=18`
	);
...
async function getTopArtists() {
	const response = await fetch(
		`https://ws.audioscrobbler.com/2.0/?method=chart.gettopartists&api_key=${API_key}&format=json&limit=18`
	);
	const data = await response.json();
	return data;
}
...
async function getTrackDetails(artist, track) {
	try {
		const response = await fetch(
			`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=${API_key}&artist=${artist}&track=${track}&format=json`
		);
		const data = await response.json();
		return data;
	} catch (error) {
...
async function renderArtistDetails() {
	...
	const spotifyAccessToken = await getSpotifyAccessToken();
	const [fetchedArtistDetails, responseSpotify] = await Promise.all([
		getArtistDetails(artistParam),
		fetch(
			`https://api.spotify.com/v1/search?type=artist&q=${artistParam}&decorate_restrictions=false&include_external=audio&limit=1&access_token=${spotifyAccessToken}`
		),
	]);
	...
	const dataTopTracksByArtist = await fetch(
		`https://api.spotify.com/v1/artists/${dataSpotify.artists.items[0].id}/top-tracks?access_token=${spotifyAccessToken}`
	).then((res) => res.json());
	...
	otherTracksToRender.forEach((track) => {
		...
		linkToTrack.setAttribute(
			"href",
			`./track?name=${track.name.split(" ").join("_")}&artist=${track.artists[0].name.split(" ").join("_")}`
		);
...
async function renderTrackDetails() {
	...
	const spotifyAccessToken = await getSpotifyAccessToken();
	const [fetchTrackDetails, dataSpotify] = await Promise.all([
		getTrackDetails(artistName, trackName),
		fetch(
			`https://api.spotify.com/v1/search?type=track&q=${trackName}&artist=${artistName}&decorate_restrictions=false&best_match=true&include_external=audio&limit=1&access_token=${spotifyAccessToken}`
		).then((res) => res.json()),
	]);
	...
	const dataTopTracksByArtist = await fetch(
		`https://api.spotify.com/v1/artists/${dataSpotify.best_match.items[0].artists[0].id}/top-tracks?access_token=${spotifyAccessToken}`
	).then((res) => res.json());
...
const artistAndTrackGen = (arr) =>
	async function* () {
		const spotifyAccessToken = await getSpotifyAccessToken();
		for (let i = 0; i < arr.length; i++) {
			const response = await fetch(
				`https://api.spotify.com/v1/search?type=track&q=${arr[i].trackName}&artist=${arr[i].artist}&decorate_restrictions=false&best_match=true&include_external=audio&limit=1&access_token=${spotifyAccessToken}`
			);
			const data = await response.json();
...
async function renderTopTracks() {
	...
	for await (const track of trackGen()) {
		const element = createCard(
			track.data.best_match.items[0].album.images[0].url,
			topTracks[track.index].name,
			topTracks[track.index].artist.name,
			topTracks[track.index].listeners,
			topTracks[track.index].playcount
		);
...
// Function to create generator to get fetched request in iteration
const artistsGen = (arr) =>
	async function* () {
		const spotifyAccessToken = await getSpotifyAccessToken();
		for (let i = 0; i < arr.length; i++) {
			const response = await fetch(
				`https://api.spotify.com/v1/search?type=artist&q=${arr[i]}&decorate_restrictions=false&include_external=audio&limit=1&access_token=${spotifyAccessToken}`
			);
			const data = await response.json();
...
async function renderTopArtists() {
	...
	for await (const artist of artistsFetched()) {
		const element = createArtistCard(
			artist.data.artists.items[0].images[0].url,
			artistNames[artist.index],
			artist.data.artists.items[0].followers.total
		);
...
function createCard(imgUrl, trackName, artistName, listeningCount, playCount) {
	...
	div.addEventListener("click", () => {
		const url = new URL(
			`./track?name=${trackName.split(" ").join("_")}&artist=${artistName.split(" ").join("_")}`,
			location.href
		);
		navigate(url);
	});
